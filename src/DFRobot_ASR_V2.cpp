/*!
 * @file DFRobot_ASR_V2.cpp
 * @brief Implemention of DFRobot_ASR_V2 class
...
DFRobot_ASR_V2::DFRobot_ASR_V2()
{
}

bool DFRobot_ASR_V2::begin(uint8_t duration)
{
  uint8_t data[2] = { 0 };
  data[0] = duration;
...
void DFRobot_ASR_V2::setWakeupTime(uint8_t duration)
{
  writeReg(WAKEUP_TIME_REG, &duration, 1);
}

bool DFRobot_ASR_V2::addCommandWord(uint8_t num, String str)
{
...
void DFRobot_ASR_V2::delCommandWord(uint8_t num)
{
  writeReg(DEL_CMD_BY_ID_REG, &num, 1);
}

void DFRobot_ASR_V2::delCommandWord(String str)
{
  int length = str.length();
...
uint8_t DFRobot_ASR_V2::getKeywordID(void)
{
  uint8_t id = 0xFF;
...
DFRobot_ASR_V2_I2C::DFRobot_ASR_V2_I2C(TwoWire* pWire, uint8_t addr)
{
  _pWire = pWire;
  _deviceAddr = addr;
}

bool DFRobot_ASR_V2_I2C::begin(uint8_t duration)
{
  _pWire->begin();
  ...
  return DFRobot_ASR_V2::begin(duration);
}

void DFRobot_ASR_V2_I2C::writeReg(uint8_t reg, void* pBuf, size_t size)
{
  if (pBuf == NULL) {
...
uint8_t DFRobot_ASR_V2_I2C::readReg(uint8_t reg, void* pBuf, size_t size)
{
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
...
DFRobot_ASR_V2_UART::DFRobot_ASR_V2_UART(SoftwareSerial* sSerial, uint32_t Baud)
{
  _serial = sSerial;
  _baud = Baud;
  // _serial->begin(_baud);
...
#else
DFRobot_ASR_V2_UART::DFRobot_ASR_V2_UART(HardwareSerial* hSerial, uint32_t Baud, uint8_t txpin, uint8_t rxpin)
{
  _serial = hSerial;
  _baud = Baud;
  _txpin = txpin;
...
bool DFRobot_ASR_V2_UART::begin(uint8_t duration)
{
#ifdef ESP32
...
void DFRobot_ASR_V2_UART::writeReg(uint8_t reg, void* pBuf, size_t size)
{
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
...
uint8_t DFRobot_ASR_V2_UART::readReg(uint8_t reg, void* pBuf, size_t size)
{
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
